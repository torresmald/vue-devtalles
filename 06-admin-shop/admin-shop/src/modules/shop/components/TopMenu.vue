    ...
    <div class="container mx-auto flex max-w-6xl flex-wrap items-center justify-between">
      ...
      <div class="mt-2 sm:mt-0 sm:flex md:order-2">
        <template v-if="!authStore.isAuthenticated">
          <button @click="$router.push({ name: 'login' })" :to="{name: 'login'}"
            class="rounde mr-3 hidden border border-blue-700 py-1.5 px-6 text-center text-sm font-medium text-blue-700 focus:outline-none focus:ring-4 focus:ring-blue-300 md:inline-block rounded-lg cursor-pointer">Login</button>
          <RouterLink @click="$router.push({ name: 'register' })" :to="{name: 'register'}"
            class="rounde mr-3 hidden bg-blue-700 py-1.5 px-6 text-center text-sm font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 md:mr-0 md:inline-block rounded-lg cursor-pointer">Register</RouterLink>
        </template>
        <template v-else>

          <RouterLink :to="{name: 'admin'}"
          v-if="authStore.isAdmin"
          class="rounde mr-3 hidden border border-blue-700 py-1.5 px-6 text-center text-sm font-medium text-blue-700 focus:outline-none focus:ring-4 focus:ring-blue-300 md:inline-block rounded-lg cursor-pointer">Admin</RouterLink>
          <button  @click="authStore.logout()" type="button"
          class="rounde mr-3 hidden bg-blue-700 py-1.5 px-6 text-center text-sm font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 md:mr-0 md:inline-block rounded-lg cursor-pointer">Logout</button>
        </template>
